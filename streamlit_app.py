...
def fetch_platform_metrics():
    metrics = {
        ...
        "light_equipment": 27, # معدات خفيفة وأدوات إنتاج (تخصص التجار الستة)
        "heavy_equipment": 14, # آليات ومعدات ثقيلة
        "generators": 9,      # طاقة ومولدات كهرباء
        "logistics": 22,      # خدمات وتموين لوجستي
        "gold_price": "في انتظار التحديث"
    }
    try:
        # جلب آخر سعر للذهب من قاعدة البيانات مربوطاً بالبورصة العالمية
        r_price = requests.get(f"{API_URL}/api/v1/prices", timeout=5)
        if r_price.status_code == 200:
            prices = r_price.json()
            if prices:
                # عرض السعر العالمي الأخير المحدث لحظياً
...
# =========================================================================
# 2. بوابة المشتري: طلب معدة + إتمام الصفقة وعرض الحساب البنكي وسند الضمان
# =========================================================================
                ...
                if not bids:
                    ...
                    for idx, bid in enumerate(bids):
                        with st.container(border=True):
                            ...
                            if st.button(f"🤝 الموافقة على العرض وتلقي بيانات الاتصال وسند الصفقة ({bid['id']})"):
                                # تحديث حالة العرض وقبول الصفقة لإظهار الأرقام والعمولة بالتراضي
                                requests.put(f"{API_URL}/api/v1/bids/{bid['id']}/accept", timeout=5)
                                st.balloons()
                                st.success("🎉 مبروك! تم موافقتك المبدئية على العرض بنجاح. تفاصيل الصفقة والضمان المالي بالأسفل:")
                                
                                # عرض الحساب البنكي والتعليمات بشكل واضح بارز
                                st.warning("""
                                💰 **ملاحظة مالية هامة للتأكيد والاعتماد النهائي:**
                                يرجى تحويل قيمة عمولة المنصة المتفق عليها بالتراضي مع التاجر إلى الحساب التالي:
                                - **بنك الخرطوم (بنكك):** 9291029
                                - **باسم:** أيمن محمد سليمان
                                - **تأكيد الدفع:** يرجى إرسال صورة/إشعار التحويل فوراً عبر الواتساب إلى الرقم: **0912236979** لإتمام تفعيل وثيقة التسليم الميداني.
                                """)
                                
                                # وثيقة المراسلات الرسمية التي تستخدم كدليل وضمان
                                st.markdown("### 📜 وثيقة اتفاق رسمي وسند ضمان الصفقة (قابل للنسخ):")
                                st.code(f"""
================================================================
📜 وثيقة اعتماد رسمي - منصة تعدين السودان الرقمية
================================================================
رقم الطلب المرجعي للمنصة: #{st.session_state.last_order_id}
معرف العرض المعتمد: #{bid['id']}
تاريخ وجلسة الاعتماد: 2026

[تفاصيل الصفقة التجارية]:
- نوع الآلية/الخدمة: تم التخصيص والموافقة بنجاح
- السعر الفعلي المعتمد: {bid['price']} {bid.get('currency', 'SDG')}
- موقع المعاينة والمطابقة الميدانية: {bid.get('location', 'موقع التاجر')}

[الالتزامات والضمانات المالية للعمولة]:
- الطرف الملتزم قانونياً بالعمولة: التاجر المورد للمخزن
- آلية السداد: تحويل بنكي مباشر لحساب المنصة المعتمد
- الحساب البنكي المعتمد (بنكك): 9291029 (أيمن محمد سليمان)
- هاتف الاستلام وتأكيد الإشعارات (واتساب): 0912236979

تعتبر هذه المراسلة المسجلة إلكترونياً بمثابة دليل رسمي وضمان كامل
لحفظ حقوق المنصة وتنسيق الصفقة بين المشتري وصاحب المخزن المورد.
================================================================
""", language="text")
        except:
        ...
        with st.form("order_form"):
            ...
            buyer_contact = st.text_input("📞 رقم الواتساب أو البريد الإلكتروني (يكفي أحدهما لإكمال الطلب)")
            
            # تصنيفات المجموعات اللوجستية المعدلة حسب طبيعة السوق في السودان
            ...
            # حقل نصي يظهر إذا اختار صنف غير مدرج لتوسيع الحرية للتجار والزبائن
            ...
            submit = st.form_submit_button("🚀 تعميم الطلب في غرف الموردين (المخازن المغلقة)")
            if submit:
...
# =========================================================================
# 3. بوابة التجار الـ 7 لتقديم العروض السرية (المخازن المغلقة والمنافسة المخفية)
# =========================================================================
                ...
                for order in orders:
                    with st.expander(f"📋 طلب آلية/خدمة مطلوبة - معرف الطلب #{order['id']}"):
                        ...
                        with st.form(f"bid_form_{order['id']}"):
                            ...
                            loc = st.text_input("📍 الموقع الحالي للمعدة في السودان (مثال: أمدرمان، سوق الخلاء)")
                            ...
                            if submit_bid:
                                if m_phone and loc and price > 0:
                                    ...
                                    if requests.post(f"{API_URL}/api/v1/bids", json=bid_payload, timeout=5).status_code in [200, 201]:
                                        st.success("🎯 تم إرسال عرضك المنافس بنجاح وبشكل سري تماماً للمستثمر وجاهز للتوثيق الإداري.")
                                else:
                                    st.warning("⚠️ الحقول الأساسية (رقم الهاتف، الموقع، والسعر) مطلوبة لإتمام المزايدة.")
    except:
        st.error("⚠️ خطأ في الاتصال بالمنصة الخلفية")

# =========================================================================
# 4. بقية الأقسام الأصلية (مواقع التعدين وبورصة الذهب المحلية والعالمية)
# =========================================================================
    ...
    with col_b:
        ...
        st.caption("يتم ربط المؤشر بالأسعار العالمية ويتم إدخال السعر المحلي للمنطقة لخلق خاصية تنافسية.")
        gold_region = st.text_input("📍 المنطقة الخاصة بالتسعير المحلي (مثال: أبو حمد):")
        ...
        if st.button("🔄 تحديث السعر والبورصة الآن"):
            if gold_region and g_price > 0:
                ...
                if requests.post(f"{API_URL}/api/v1/prices", json=payload_price, timeout=5).status_code in [200, 201]:
                    st.success("🎉 تم تحديث البورصة حية في لوحة المؤشرات لجميع المستثمرين.")
                    st.cache_data.clear()
